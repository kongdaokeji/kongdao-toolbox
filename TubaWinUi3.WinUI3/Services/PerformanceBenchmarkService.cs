...
public static class PerformanceBenchmarkService
{
	...
	/// <summary>
	/// P1-8：自动选择默认 GPU —— 多 GPU 笔记本上 index 0 常是核显，
	/// 按独显特征（GeForce/NVIDIA/Quadro/Radeon RX/Pro/Arc）优先挑选；无独显则取第一个。
	/// </summary>
	internal static int PickDefaultGpuIndex()
	{
		int DiscretePriority(string name)
		{
			if (name.Contains("GeForce", StringComparison.OrdinalIgnoreCase)) return 0;
			if (name.Contains("NVIDIA", StringComparison.OrdinalIgnoreCase)) return 0;
			if (name.Contains("Quadro", StringComparison.OrdinalIgnoreCase)) return 0;
			if (name.Contains("Radeon RX", StringComparison.OrdinalIgnoreCase)) return 0;
			if (name.Contains("Radeon Pro", StringComparison.OrdinalIgnoreCase)) return 0;
			if (name.Contains("Arc A", StringComparison.OrdinalIgnoreCase)) return 1;
			...
			return 2; // 其余 AMD/未知：核显可能性高，排独显之后
		}
		...
		try
		{
			...
			if (gpus.Count == 0)
				gpus = LiteMonitorService.GetAvailableGpus()
					.Select(g => new FurMarkGpuInfo { Index = g.Index, Name = g.Name })
					.ToList();
			if (gpus.Count == 0) return 0;
			int bestIdx = 0, bestPri = int.MaxValue;
			for (int i = 0; i < gpus.Count; i++)
			{
				int pri = DiscretePriority(gpus[i].Name);
				if (pri < bestPri) { bestPri = pri; bestIdx = i; }
			}
			// FurMarkGpuInfo.Index 与列表下标可能不一致，返回列表下标
			return bestIdx;
		}
		catch
		{
	...
	public static GpuBenchmarkResult RunGpuBenchmarkFurMark(int durationSec, IProgress<BenchmarkProgress>? progress, CancellationToken ct, int gpuIndex = -1, string? gpuName = null)
	{
		// P1-8：gpuIndex<0 表示自动选择 —— 默认优先独显而非 index 0（常为核显）
		if (gpuIndex < 0)
			gpuIndex = PickDefaultGpuIndex();
		var sw = Stopwatch.StartNew();
