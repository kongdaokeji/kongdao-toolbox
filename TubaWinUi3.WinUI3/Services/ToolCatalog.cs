...
public static class ToolCatalog
{
    ...
    public static ArchOption? PickPreferredArchOption(IReadOnlyList<ArchOption> options, ArchOption? fallback = null)
    {
        ...
        // P1-4 防御：tools.json 标注或文件名识别可能把 arch 标错（如 ARM64 变体被标成 x64），
        // 文件名探测（cpuz_x64.exe 等）通常比标签更可靠 —— 标签与文件名冲突时以文件名为准。
        string EffectiveArch(ArchOption o)
        {
            try
            {
                var fromName = DetectArch(Path.GetFileNameWithoutExtension(o.Path));
        ...
        {
            var match = options.FirstOrDefault(o => EffectiveArch(o).Equals(arch, StringComparison.OrdinalIgnoreCase));
            if (match is not null)
            {
                Services.Agent.AgentDebugLog.Info(
                    $"[ArchPick] 选中 {match.Name} effectiveArch={EffectiveArch(match)} label={match.Arch} os={OSArch}");
                return match;
            }
        }

        var fb = fallback ?? options[0];
        Services.Agent.AgentDebugLog.Info(
            $"[ArchPick] 无精确匹配，回退 {fb.Name} label={fb.Arch} os={OSArch}");
        return fb;
    }
