...
public sealed partial class PerformanceBenchmarkPage : Page
{
	...
	{
		// P1-8：独显优先。AMD 核显（Radeon 6xxM/7xxM/8xxM）也含 "AMD"，
		// 必须先识别独显特征，再把模糊的 "AMD"/核显特征排后。
		...
		if (name.Contains("Radeon(TM) Graphics", StringComparison.OrdinalIgnoreCase)) return 2; // AMD APU 核显
		if (name.Contains("Radeon ", StringComparison.OrdinalIgnoreCase) &&
		    System.Text.RegularExpressions.Regex.IsMatch(name, @"Radeon\s+\d{3}M\b")) return 2;   // Radeon 680M 等核显
		if (name.Contains("Radeon", StringComparison.OrdinalIgnoreCase)) return 0; // 其余 Radeon 视为独显（RX/Pro 已上面命中）
		if (name.Contains("AMD", StringComparison.OrdinalIgnoreCase)) return 2;
