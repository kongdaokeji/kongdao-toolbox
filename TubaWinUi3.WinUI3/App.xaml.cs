...
public partial class App : Application
{
    ...
    public App()
    {
        ...
        // RequestedTheme 会导致文字刷与窗口表面/背景刷分属两套主题（白底白字）。
        RequestedTheme = ApplicationTheme.Dark;
        Services.BrandAccent.Apply(); // 深空玻璃配色注入（Light/Dark 字典覆盖）
    ...
    private void OnUnobservedTaskException(object? sender, UnobservedTaskExceptionEventArgs e)
    {
        ...
        // P1-1：已知后台异常补充写入 app_crash.log（而非弹错误页），便于排障且避免反复打断。
        try
        {
            var logPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "app_crash.log");
            var stamp = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] UnobservedTaskException:\n{e.Exception}\n";
            System.IO.File.AppendAllText(logPath, stamp);
        }
        catch
        {
        }
    ...
    {
        // P1-1：错误页单例守卫 + 短时间窗去重，避免同一波异常反复弹窗把应用卡死。
        // 若错误页已打开则直接激活已有实例，不新建。
        var now = DateTime.UtcNow;
        lock (_errorWindowGuard)
        {
            if (_errorWindow is not null)
            {
                var existing = _errorWindow;
                _window?.DispatcherQueue.TryEnqueue(() => existing.Activate());
                return;
            }

            // 5 秒内已弹出过则忽略后续（同一根因的连锁异常）
            if (now - _lastErrorWindowAt < TimeSpan.FromSeconds(5))
                return;

            _lastErrorWindowAt = now;
        }
    ...
    private static readonly object _errorWindowGuard = new object();
    private static Pages.ErrorWindow? _errorWindow;
    private static DateTime _lastErrorWindowAt = DateTime.MinValue;
}
